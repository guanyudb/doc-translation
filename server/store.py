...
_SCHEMA_SQL_PATH = pathlib.Path(__file__).parent / "schema.sql"
...
def ensure_schema() -> None:
    ...
    The DDL is the shared server/schema.sql (also read by setup/postdeploy.py),
    so the schema is defined in exactly one place. Executed as one multi-statement
    command; keep every statement in that file semicolon-terminated."""
    sql = _SCHEMA_SQL_PATH.read_text(encoding="utf-8").replace("{schema}", config.PGSCHEMA)
